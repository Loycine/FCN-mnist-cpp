...
namespace detail 
{
    ...
    inline void parse_one_image(std::ifstream &ifs, mnist_header const &header,
            flt_type const scale_min,  flt_type const scale_max,
            int const x_padding,       int const y_padding,     std::vector<flt_type> &dst) 
    {
        ...
        for (uint32_t y = 0; y < header.num_rows; y++)
            for (uint32_t x = 0; x < header.num_cols; x++)
                dst[width * (y + y_padding) + x + x_padding] =
                    (image_vec[y * header.num_cols + x] / flt_type(255)) * 
...
inline void parse_mnist_images(const std::string &image_file, 
        std::vector< one_image > *images, 
        flt_type const scale_min, flt_type const scale_max, 
        int const x_padding, int const y_padding)
{
    ...
    {
        one_image image;
        detail::parse_one_image(ifs, header, scale_min, scale_max, 
